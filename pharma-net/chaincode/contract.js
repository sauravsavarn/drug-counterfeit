...
class PharmanetContract extends Contract {
        ...
        if (listOfAssetsLength == parsedPurchaseOrder.quantity) {
          ...
          for (let i = 0; i <= listFromCommandLine.length - 1; i++) {
            if (validDrugId) {
              ...
              console.log("serialnumberOfTheDrug = " + serialnumberOfTheDrug + " - productDrugID = " + productDrugID);
  ...
  async updateShipment(ctx, buyerCRN, drugName, transporterCRN) {
    ...
    if (transportForUpdateShipmentCRN === transporterCRN) {
      ...
      let generatedShipmentCompositeKey = ctx.stub.createCompositeKey("org.pharma-network.pharmanet.shipment", [
        buyerCRN,
        drugName,
      ]);

      //fetch the shipment details based on the shipment-compositeKey
      ...
      console.log("generatedShipmentCompositeKey => " + generatedShipmentCompositeKey.toString());
      ...
      if (parsedShipmentData.transporter === generateTransporterForShipmentUpdation) {
        ...
        // console.log("<<<ParsedShipmentData>>> : " + JSON.stringify(parsedShipmentData));
        ...
        // console.log("contract - parsedShipmentData : " + drugsInShipment);
        // console.log("contract - parsedShipmentData.length : " + drugsInShipment.length);

        //create an array to store the shipped details of SKU's
        let drugsShipped = [];

        //
        for (let i = 0; i <= drugsInShipment.length - 1; i++) {
          console.log("drugsInShipment => " + drugsInShipment[i]);
          ...
          //send the json response to the caller
          // console.log("JSONDrugDetailsForUpdation stringify : " + JSON.stringify(JSONDrugDetailsForUpdation));
          //return JSON.stringify(JSONDrugDetailsForUpdation);
          // return modifiedDrugDetailsObject;

          //add the shipped details to the array 'drugsShipped'.
          drugsShipped.push(JSON.stringify(JSONDrugDetailsForUpdation));
        }

        //return the shipment 'Delivered' with the details to the caller.
        // console.log("shipmentDataBuffer stringify : " + JSON.stringify(parsedShipmentData));
        console.log("shipmentDataBuffer stringify => " + drugsShipped);
        // return JSON.stringify(parsedShipmentData);  
        return JSON.stringify(drugsShipped);  

      }
      // console.log("shipmentDataBuffer stringify : " + shipmentDataBuffer.toJSON());
      console.log("shipmentDataBuffer => " + shipmentDataBuffer.toString());
  ...
  async retailDrug(ctx, drugName, serialNo, retailerCRN, customerAadhar) {
    ...
    if (JSONDrugCompositeKeyForSearch.owner === generateRetailerCompanyIDForOwner) {
      ...
      return "{\"message\":\"SOrry you are not the owner of this drug\"}";
    }
